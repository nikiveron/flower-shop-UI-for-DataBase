...
{
    public partial class DeleteOtherForm : Form
    {
        public DeleteOtherForm()
        {
            InitializeComponent();
        }

        private void DeleteOtherForm_Load(object sender, EventArgs e)
        {
        ...
        {
            int id;
            if (!int.TryParse(textBoxOtherId.Text, out id))
            {
                MessageBox.Show("Введите корректный числовой ID товара.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            NpgsqlCommand command = new NpgsqlCommand("DELETE FROM other WHERE Id = @ic;", DB.GetConnection());
